...
class CliOne
{
    ...
    public function showHelp(CliOneParam $parameter, bool $verbose): void
    {
        ...
        if ($verbose) {
            if (count($parameter->alias) > 0) {
                $jenc = $this->jsonEncodeNoError($parameter->alias);
                $this->showLine("  <yellow>Aliases: </yellow> " . $jenc);
            }
    ...
    /**
     * Used internally to encode json values.
     * @param mixed $value
     * @param int $flags
     * @param mixed $default
     * @return false|mixed|string
     */
    protected function jsonEncodeNoError($value,int $flags=JSON_THROW_ON_ERROR,$default='') {
        try {
...
':
    ...
    public function saveData(string $filename, $content, string $defaultExtension = '.config.php'): string
    {
        ...
        $contentData .= $this->jsonEncodeNoError($content,JSON_THROW_ON_ERROR | JSON_PRETTY_PRINT);
        try {
    ...
    public function showparams(): void
    {
        foreach ($this->parameters as $parameter) {
            $this->showLine("$parameter->key = [" .
                $this->jsonEncodeNoError($parameter->default). "] value:" .
    ...
    public function showParamValue(CliOneParam $parameter): string
    {
        ...
        if (is_array($parameter->value)) {
            return $this->jsonEncodeNoError($parameter->value);
        }
        return $parameter->value;
    }
        ...
        if (self::$fakeReadLine !== null) {
            ...
            if (self::$fakeReadLine[0] >= count(self::$fakeReadLine)) {
                if (self::$throwNoInput) {
                    throw new RuntimeException('Test incorrect, it is waiting for read more CliOne::$fakeReadLine ' .
                        $this->jsonEncodeNoError(self::$fakeReadLine)
                    );
                }
    ...
    protected function showPattern(CliOneParam $parameter, string $key, $value, string $selection, int $colW, string $prefix, string $pattern): string
    {
        ...
        $valueToShow = (is_object($value) || is_array($value)) ? $this->jsonEncodeNoError($value) : $value;
        if (is_array($value)) {
                    ...
                    if ($this->debug && strpos($origInput, '??') === 0) {
                        switch ($origInput) {
                            case strpos($origInput, '??save') === 0:
                                ...
                                $jsonTxt = $this->jsonEncodeNoError($this->debugHistory, JSON_THROW_ON_ERROR | JSON_PRETTY_PRINT);
                                $r = @file_put_contents($file . '.json', $jsonTxt);
                                if ($r === false) {
