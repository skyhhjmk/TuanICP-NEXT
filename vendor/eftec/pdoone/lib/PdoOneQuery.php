...
class PdoOneQuery
{
    ...
    public function toMeta(?string $sql = null, array $args = [])
    {
        ...
        if ($sql === null) {
            ...
            /** @var PDOStatement $stmt */
            $stmt = $this->runRawQuery($sql, $args, false, $this->useCache !== false, $this->cacheFamily, $this->fetchMode, $this->fetchArgument);
        }
    ...
    public function sqlGen(bool $resetStack = false): string
    {
        ...
        if (!in_array('from', $words, true)) { // is from missing?
            if($this->from) {
                $sql .= ' from ' . $this->from;
            } else if($this->parent->databaseType==='oci') {
                $sql .= ' from dual ';
            }
        } else {
            $sql .= $this->from;
     ...
     *  $this->where('field',[20])
     *  $this->where('field=?',[20])
     *  $this->where('field between ? and ?',[10,20])
     ...
     * **Example:**
     * ```
     * where( 'field=1' ) // static (don't use field=$variable unless the variable is sanitized)
     ...
     * <b>Note:</b> The prefix (PdoOne->$prefixTable) is considered only if the table is defined alone.<br>
     * **Example:**
     * ```
     * $this->from('table')
     * $this->from('table alias')
     * $this->from('table1,table2')
     * $this->from('table1,table2','dbo')
     * $this->from('table1 inner join table2 on table1.c=table2.c')
     * ```
     *
     ...
     * $this->count('from table') // select count(*) from table
     * $this->count('from table where condition=1') // select count(*) from table where condition=1
     * $this->count('from table','col') // select count(col) from table
     * ```
     *
     ...
     * **Example:**
     * ```
     * $this->min('from table','col') // select min(col) from table
     * $this->from('table')->min('col') // select min(col) from table
     * $this->from('table')->min('','col') // select min(col) from table
     * ```
     * @param string $sql
     * @param string $arg
     ...
     * $this->from("table")->set("type=?",6) // automatic
     * ```
     *
                               ...
                               $identityColumn = null)
    {
        ...
        if($this->parent->databaseType === 'pgsql' && $identityColumn !== null) {
            //$param[] = [':ID_' . $identityColumn, '0', 1, null];
            $sql .= ' returning ' . $identityColumn;
        }
        $resultInsert=$this->runRawQuery($sql, $param, true, $this->useCache !== false, $this->cacheFamily, $this->fetchMode, $this->fetchArgument);
        ...
        if($this->parent->databaseType === 'pgsql') {
            return $identityColumn === null ? null : $resultInsert[0][$identityColumn];
        }
     ...
     */
    public function setFechMode(int $fetchMode, $fetchArgument = null): PdoOneQuery
    {
     ...
     *                                            cache.
     * @param int|null             $fetchMode     The mode to fetch. By default is PDO::FETCH_ASSOC
     * @param null                 $fetchArgument The argument of the fetch mode
     ...
     */
    public function runRawQuery(string $rawSql, ?array $params = null, ?bool $returnArray = true,
                                bool   $useCache = false, $cacheFamily = null, ?int $fetchMode = null, $fetchArgument = null)
    {
        return $this->parent->runRawQuery($rawSql, $params, $returnArray, $useCache, $cacheFamily,
            $fetchMode ?? $this->fetchMode, $fetchArgument ?? $this->fetchArgument);
    }
    ...
    )
    {
        ...
        $this->beginTry();
        $stmt = $this->runRawQuery($sql, $param, false, $this->useCache !== false, $this->cacheFamily, $this->fetchMode, $this->fetchArgument);
        $this->builderReset(true);
        if ($this->endtry() === false) {
            return false;
        }
    ...
    public function now(): ?string
    {
        ...
        try {
            $r = $this->runRawQuery($sql, null, true, false, null, $this->fetchMode, $this->fetchArgument);
        } catch (Exception $e) {
